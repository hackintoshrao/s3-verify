...
var commonBucketActions = CreateStringSet("s3:GetBucketLocation")

// Read only bucket actions.
var readOnlyBucketActions = CreateStringSet("s3:ListBucket")

// Write only bucket actions.
var writeOnlyBucketActions = CreateStringSet("s3:ListBucketMultipartUploads")

// Read only object actions.
var readOnlyObjectActions = CreateStringSet("s3:GetObject")

// Write only object actions.
var writeOnlyObjectActions = CreateStringSet("s3:AbortMultipartUpload", "s3:DeleteObject", "s3:ListMultipartUploadParts", "s3:PutObject")
...
func newBucketStatement(policy BucketPolicy, bucketName string, prefix string) (statements []Statement) {
	...
	bucketResource := CreateStringSet(awsResourcePrefix + bucketName)

	statement := Statement{
		Actions:   commonBucketActions,
		Effect:    "Allow",
		Principal: User{AWS: CreateStringSet("*")},
	...
	if policy == BucketPolicyReadOnly || policy == BucketPolicyReadWrite {
		...
		if prefix != "" {
			...
			condKeyMap.Add("s3:prefix", CreateStringSet(prefix))
...
func newObjectStatement(policy BucketPolicy, bucketName string, prefix string) (statements []Statement) {
	...
	statement := Statement{
		Effect:    "Allow",
		Principal: User{AWS: CreateStringSet("*")},
		Resources: CreateStringSet(awsResourcePrefix + bucketName + "/" + prefix + "*"),
		Sid:       "",
	}
...
func GetPolicy(statements []Statement, bucketName string, prefix string) BucketPolicy {
	...
	for _, s := range statements {
		matchedObjResources := NewStringSet()
